...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bgStrip = new FlxSprite().makeGraphic(FlxG.width, Std.int(FlxG.height / 1.5), FlxColor.fromRGB(77, 114, 151, 200));
		bgStrip.screenCenter();
		add(bgStrip);
		...
		gfDance = new FlxSprite(FlxG.width * 0.5, FlxG.height * 0.05);
		gfDance.frames = Paths.getSparrowAtlas('menus/base/title/chrismenubounce');
		gfDance.animation.addByPrefix('dance', "CHRISBOUNCE", 24);
		...
		ngSpr = new FlxSprite(0, 0).loadGraphic(Paths.image('menus/base/title/penta'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function beatHit()
	{
		...
		{
			case 1:
				createCoolText(['A game by', 'ninjamuffin', 'phantomArcade', 'kawaisprite', 'evilsker']);
			...
			// credTextShit.text += '\nNight';
			case 17:
				addMoreText('Chris'); // credTextShit.text += '\nFunkin';

			...
			default:
				...
				for (section in noteData)
				{
					...
					for (songNotes in section.sectionNotes)
					{
						...
						// create the new note
						var mod = PlayState.assetModifier;

						if (songData.song.toLowerCase() == "burnout" && daBeats > 73)
						{
							mod = "NOTE_assetsblu";
						}

						var swagNote:Note = ForeverAssets.generateArrow(mod, daStrumTime, daNoteData, 0, daNoteAlt);
						// set note speed
						...
						{
							oldNote = unspawnNotes[Std.int(unspawnNotes.length - 1)];
							var sustainNote:Note = ForeverAssets.generateArrow(PlayState.assetModifier, daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet,
								daNoteData, 0, daNoteAlt, true, oldNote);
